...
flowchart LR
  ...
  pkg_session_telemetry_file --> svc_sessionTelemetry
  ...
  svc_authorization --> pkg_command_login
...
| `ctx.authorization` | `seam` | [`authorization`](../packages/credentials/authorization) | - | [`llm-pi-ai`](../packages/llm/llm-pi-ai), [`command-login`](../packages/credentials/command-login) | - | flow 由知道如何取得某份凭据的插件注册，并以其写入的记录为键；seam 拥有这段对话与"每个键同时只跑一次尝试"的生命周期，而非协议本身。 |
| `ctx.sessionTelemetry` | `seam` | [`session-telemetry`](../packages/session/session-telemetry) | [`session-telemetry-file`](../packages/session/session-telemetry-file) | - | - | 该 seam 捕获会话记录、进行脱敏并交给一个后端；随附后端把记录追加到本地的按会话文件中，因此默认不会有任何内容离开当前进程。 |
