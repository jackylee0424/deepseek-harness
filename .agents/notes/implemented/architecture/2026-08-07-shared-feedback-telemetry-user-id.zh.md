...
`@deepseek-ai/dsh-anonymous-user-id` 负责 `getOrCreateAnonymousUserId()` 和 `$DSH_HOME/.anonymous-user-id` 存储约定。`/feedback` 的成功确认先报告 `Feedback recorded for session {sessionId}`，再在第二行显示 `Anonymous user: {userId}`；自[移除远程遥测](../simplification/2026-09-02-remove-remote-telemetry.zh.md)删除了以 Resource `user.id` 携带该 id 的 OpenTelemetry 后端以及 DeepSeek 的 `x-deepseek-harness-user-id` 标头之后，它是唯一随附的消费方。系统在获取 id 前拒绝无效反馈，因此空命令不会创建 `.anonymous-user-id`。
...
- 一个 harness home 只有一个匿名 id，由反馈确认报告，并可供部署方自行挂载的任何后端使用。
