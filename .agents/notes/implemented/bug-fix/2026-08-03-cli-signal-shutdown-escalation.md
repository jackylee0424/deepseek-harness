...
The OpenTelemetry backend's regression test held a real OTLP request open after timer export began and pinned that Cordis disposal returned at `shutdownTimeoutMillis` despite the SDK's `forceFlush()` remaining pending; [Remove remote telemetry](../simplification/2026-09-02-remove-remote-telemetry.md) deleted that backend and its test, while the launcher's five-second escalation bound remains in force.
