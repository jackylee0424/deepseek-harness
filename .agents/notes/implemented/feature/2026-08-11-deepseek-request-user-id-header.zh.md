...
该决策已不再生效：[移除远程遥测](../simplification/2026-09-02-remove-remote-telemetry.zh.md)从每个 DeepSeek 请求中移除了 `x-deepseek-harness-user-id` 与 `x-deepseek-harness-session-id` 标头。以下各节记录的是该移除之前已交付的决策。
