...
- [插件包清单](../plugin-package-inventory-deepseek/README.zh.md)——可选启用的 `dsh_plugin_packages` 贡献，随附 profile 默认禁用。
