...
基础组合包挂载原生 DeepSeek 适配器、settings 与凭据提供方、稳定的 `web_search` 和 `web_fetch`、仅限公网的 HTTP fetch 提供方，以及本地会话遥测。提供方凭据依次从继承环境、`$DSH_HOME/.credentials.yaml`、调用目录的 `.env` 和 `$DSH_HOME/.env` 解析；受管文档从不物化进 `process.env`，而两个 `.env` 文件都是普通启动环境层。搜索使用 `DEEPSEEK_API_KEY` 并接受 `DEEPSEEK_SEARCH_BASE_URL`。已启用的抓取调用会在所有 sandbox 与审批模式下执行，无需逐次确认；提供方会在连接前拒绝非公开目的地址。Web app 会禁用 base 工具配置项，再通过 `cordis`、`ptc` 与 `standard` agent preset 暴露相同工具。

没有任何随附 profile 会向远程 collector 上报：基础组合包把会话遥测捕获到本机的 `$DSH_HOME/telemetry/<session id>.jsonl`，DeepSeek 请求只携带产品 `User-Agent`，`plugin-package-inventory-deepseek` 与 `session-log-deepseek` 请求扩展在部署 overlay 启用之前保持关闭。相关决策见[移除远程遥测 Agent Note](../../../.agents/notes/implemented/simplification/2026-09-02-remove-remote-telemetry.zh.md)。
